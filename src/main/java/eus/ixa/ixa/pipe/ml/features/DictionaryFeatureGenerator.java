...
public class DictionaryFeatureGenerator extends CustomFeatureGenerator
  ...
  public void createFeatures(final List<String> features, final String[] tokens,
    ...
    if (this.currentSentence != tokens) {
      ...
      if (this.isBilou) {
        this.currentEntities = this.dictionary.getBilouDictionaryMatch(tokens);
      } else {
        this.currentEntities = this.dictionary.getBioDictionaryMatch(tokens);
      }
    }

    final String currentEntity = this.currentEntities.get(index);

    features.add(this.attributes.get("dict") + "=" + currentEntity);
    features.add(this.attributes.get("dict") + "," + "w=" + currentEntity + ","
        + tokens[index]);
    features.add(this.attributes.get("dict") + ",w=dict");
    if (Flags.DEBUG) {
      System.err
          .println("-> " + tokens[index] + ": " + this.attributes.get("dict")
              + "," + "w=" + currentEntity + "," + tokens[index]);
    }
  }
