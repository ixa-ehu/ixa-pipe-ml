...
/**
 * Obtain the paths listed in the pathLengths array from the Brown class. This
...
public class BrownTokenClasses {
   ...
   * {@code BrownCluster} Map token,BrownClass.
   *
   * @param token
   *          the token to be looked up in the brown clustering map
   * @param brownLexicon
   *          the Brown clustering map
   * @return the list of the paths for a token
   */
  public static List<String> getWordClasses(final String token,
      final WordCluster brownLexicon) {
    if (brownLexicon.lookupToken(token) == null) {
      return new ArrayList<String>(0);
    } else {
      final String brownClass = brownLexicon.lookupToken(token);
      final List<String> pathLengthsList = new ArrayList<String>();
  ...
  public static void printList(final List<String> classList) {
